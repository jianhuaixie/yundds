...
elseif ($_REQUEST['act'] == 'edit_type_name')
{
    ...
    /* 检查名称是否重复 */
    $sql="select count(cat_id) from .".$ecs-table('goods_type')." where seller_id='$_SESSION[seller_id]' and cat_name='$type_name'";
...
elseif ($_REQUEST['act'] == 'insert')
{
	...
	//wang 商家入驻
	$goods_type['seller_id']    = intval($_SESSION['seller_id']);
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('goods_type')." where seller_id='$_SESSION[seller_id]'";
        ...
        $sql = "SELECT t.*, COUNT(a.cat_id) AS attr_count ".
               ...
               "LEFT JOIN ". $GLOBALS['ecs']->table('attribute'). " AS a ON a.cat_id=t.cat_id where t.seller_id='$_SESSION[seller_id]'".
               "GROUP BY t.cat_id " .
