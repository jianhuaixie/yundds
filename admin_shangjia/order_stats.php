...
if ($_REQUEST['act'] == 'list')
{
    ...
    /* 取得商品总点击数量 */
    $sql = "SELECT SUM(click_count) FROM " .$ecs->table('goods') ." WHERE is_delete = 0 and seller_id='".$_SESSION['seller_id']."'";
    ...
    if ($is_multi)
    {
        ...
        foreach($start_date_arr AS $k => $val)
        {
             $sql = 'SELECT i.pay_id, p.pay_name, i.pay_time, COUNT(i.order_id) AS order_num ' .
                ...
                "AND i.add_time >= '$start_date_arr[$k]' AND i.add_time <= '$end_date_arr[$k]'  and i.seller_id='".$_SESSION['seller_id']."' ".
    ...
    else
    {
        ...
        $sql = 'SELECT i.pay_id, p.pay_name, COUNT(i.order_id) AS order_num ' .
           ...
           "AND i.add_time >= '$start_date' AND i.add_time <= '$end_date'  and i.seller_id='".$_SESSION['seller_id']."' ".
           "GROUP BY i.pay_id ORDER BY order_num DESC";
        $pay_res= $db->query($sql);
...
/*------------------------------------------------------ */
 ...
 function get_orderinfo($start_date, $end_date)
 {
    ...
    $sql = 'SELECT COUNT(*) AS unconfirmed_num FROM ' .$GLOBALS['ecs']->table('order_info').
           " WHERE order_status = '" .OS_UNCONFIRMED. "' AND add_time >= '$start_date'  and seller_id='".$_SESSION['seller_id']."' ".
    ...
    $sql = 'SELECT COUNT(*) AS confirmed_num FROM ' .$GLOBALS['ecs']->table('order_info').
           ...
           " AND add_time < '" . ($end_date + 86400) . "'  and seller_id='".$_SESSION['seller_id']."' ";
    ...
    $sql = 'SELECT COUNT(*) AS succeed_num FROM ' .$GLOBALS['ecs']->table('order_info').
           ...
           " AND add_time >= '$start_date' AND add_time < '" . ($end_date + 86400) . "'  and seller_id='".$_SESSION['seller_id']."' ";
