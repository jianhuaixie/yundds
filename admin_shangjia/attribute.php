...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
		$attr['seller_id']=$_SESSION['seller_id'];//wang 商家入驻
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $sql = "DELETE FROM " . $ecs->table('attribute') . " WHERE seller_id=$_SESSION[seller_id] and attr_id " . db_create_in($ids);
...
function get_attrlist()
{
	...
	$where =" where a.seller_id='$_SESSION[seller_id]' ";
    $where .= (!empty($filter['goods_type'])) ? " and a.cat_id = '$filter[goods_type]' " : '';
