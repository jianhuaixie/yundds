...
if ($_SERVER['REQUEST_METHOD'] == 'GET')
{
    ...
    if (!empty($_GET['act']) && $_GET['act'] == 'reset_pwd')
    {
        ...
        /* 以用户的原密码，与code的值匹配 */
        $sql = 'SELECT password FROM ' .$ecs->table('admin_user'). " WHERE user_id = '$adminid' and seller_id>0 ";
...
else
{
    ...
    if (!empty($_POST['action']) && $_POST['action'] == 'get_pwd')
    {
        ...
        $sql = 'SELECT user_id, password FROM ' .$ecs->table('admin_user').
               " WHERE user_name = '$admin_username' AND email = '$admin_email' and seller_id>0 ";
        ...
        if (!empty($admin_info))
        {
            ...
            $reset_email = $ecs->url() . SJ_ADMIN_PATH . '/get_password.php?act=reset_pwd&uid='.$admin_id.'&code='.$code;
