...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        $goods_sales_list = get_sale_list(false,$seller_id);
    ...
    $sale_list_data = get_sale_list(true,$seller_id);
...
else
{
    /* 权限判断 */
	/*wang 商家入驻*/
	$seller_id=isset($_REQUEST['sup_id'])?intval($_REQUEST['sup_id']):0;
	if($seller_id>0)
	{
    ...
    $sale_list_data = get_sale_list(false,$seller_id);
    /* 赋值到模板 */
 ...
 */
 /*wang 商家入驻*/
function get_sale_list($is_pagination = true,$seller_id=0){
    ...
    /* 查询数据的条件 wang 商家入驻*/
    $where = " WHERE og.order_id = oi.order_id and og.goods_id=g.goods_id and g.seller_id='$seller_id' and do.order_id=og.order_id and do.status=0 ".
    ...
    $sql = "SELECT COUNT(og.goods_id) FROM " .
		   ...
		   $GLOBALS['ecs']->table("delivery_order")." AS do ".
           $where;
    ...
    $sql = 'SELECT og.goods_id, og.goods_sn, og.goods_name, sum(og.goods_number) AS goods_num, (sum(og.goods_number) * og.goods_price) '.
           'AS sales_price, oi.add_time AS sales_time, oi.order_id, oi.order_sn '.
           "FROM " . $GLOBALS['ecs']->table('order_goods')." AS og, ".$GLOBALS['ecs']->table('order_info')." AS oi, ".$GLOBALS['ecs']->table('goods')." AS g, ".$GLOBALS['ecs']->table("delivery_order")." AS do ".
           $where. " group by og.goods_id ORDER BY sales_time DESC, goods_num DESC";
