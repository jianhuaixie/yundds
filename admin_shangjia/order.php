...
//ecmoban模板堂 --zhuo start 订单查询导出cvs格式
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    $sql = "SELECT MAX(order_id) FROM " . $ecs->table('order_info') . " as o WHERE order_id < '$order[order_id]' and seller_id='".$_SESSION['seller_id']."'";
    if ($agency_id > 0)
    {
    ...
    $sql = "SELECT MIN(order_id) FROM " . $ecs->table('order_info') . " as o WHERE order_id > '$order[order_id]' and seller_id='".$_SESSION['seller_id']."'";
    if ($agency_id > 0)
    {
        $sql .= " AND agency_id = '$agency_id'";
    }
    ...
    /**
     * 取得用户收货时间 以快物流信息显示为准，目前先用用户收货时间为准，后期修改TODO by Leah
     */
...
elseif ($_REQUEST['act'] == 'return_info')
{
   /* 检查权限 */
    ...
    /* 取得可用的配送方式列表 */
        ...
        foreach ($shipping_list AS $key => $shipping)
        {
            ...
            $shipping_fee = shipping_fee($shipping['shipping_code'],
                unserialize($shipping['configure']), $total['weight'], $total['amount'], $total['number']); //计算运费
    ...
    assign_query_info();
    $smarty->display('return_order_info.htm');
    exit; //
}
/*------------------------------------------------------ */
//-- 发货单列表
...
elseif ($_REQUEST['act'] == 'merge')
{
   ...
   // admin_priv('order_os_edit');
    ...
    $sql = "SELECT o.order_sn, u.user_name " .
            ...
            "WHERE o.user_id > 0 and o.seller_id='".$_SESSION['seller_id']."' " .
...
//        }
    ...
    if (($require_note && $action_note == '') || isset($show_invoice_no) || isset($show_refund))
    {
        ...
        $smarty->assign('show_refund1', isset($show_refund1)); // 是否显示退款
        $smarty->assign('anonymous', isset($anonymous) ? $anonymous : true); // 是否匿名
        $smarty->assign('order_id', $order_id); // 订单id
        $smarty->assign('rec_id', $rec_id); // 订单商品id
        $smarty->assign('ret_id', $ret_id); // 订单商品id
...
elseif ($_REQUEST['act'] == 'operate_post')
{
    ...
    elseif ('split' == $operation)
    {
		...
		//wang 商家入驻 start
        $sql ="select add_time,seller_id from ". $GLOBALS['ecs']->table('order_info') ." WHERE order_sn = '" . $delivery['order_sn'] . "'";
		$order_info=$GLOBALS['db']->GetRow($sql);
        $delivery['add_time'] =$order_info['add_time'];
		$delivery['seller_id'] =$order_info['seller_id'];
		//wang 商家入驻 end
        ...
        $filter_fileds = array(
                               ...
                               'suppliers_id', 'status', 'order_id', 'shipping_name','seller_id'
                               );
        $_delivery = array();
    ...
    elseif( 'receive_goods' == $operation ){
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_return'),     
        $arr, 'UPDATE', "rec_id = '$rec_id'");
        $arr['pay_status'] = PS_PAYED;
        $order['pay_status'] = PS_PAYED;
   ...
   elseif('complete' == $operation){
        ...
        $arr = array('return_status' => 4); //换出商品寄出
        
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_return'),     
        $arr, 'UPDATE', "rec_id = '$rec_id'"); 
        return_action( $ret_id,RF_COMPLETE, '',$action_note );
...
/**
    ...
    elseif ('refound' == $operation){
        ...
        $_REQUEST['refund_note'] = isset($_REQUEST['refund_note']) ? $_REQUEST['refund'] : ''; //退款说明
...
//        update_order($order_id, $arr);
        ...
        if ($order['pay_status'] != PS_UNPAYED)
        {
            ...
            //标记order_return 表
            $return_status = array( 'refound_status' => 1 ,'actual_return' => $refund_amount);
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_return'),
            $return_status, 'UPDATE', "rec_id = '$rec_id'");  
        ...
        if ($delivery_list)
        {
            foreach ($delivery_list as $list)
            {
                ...
                $sql_back .= " ( '" . $list['delivery_sn'] . "', '" . $list['order_sn'] . "',
                              ...
                              '" . $list['agency_id'] . "', '" . $list['invoice_no'] . "','" . $list['seller_id'] . "'
                              )";
				//wang 商家入驻 end
...
elseif ($_REQUEST['act'] == 'search_goods')
{
    ...
    if ($keyword != '')
    {
        ...
        $sql = "SELECT goods_id, goods_name, goods_sn FROM " . $GLOBALS['ecs']->table('goods') .
                " WHERE is_delete = 0 and seller_id='".$_SESSION['seller_id']."'" .
                " AND is_on_sale = 1" .
                " AND is_alone_sale = 1" .
...
function delivery_list()
{
    ...
    if ($result === false)
    {
        ...
        $where = "WHERE 1 and seller_id='".$_SESSION['seller_id']."' ";
        if ($filter['order_sn'])
        {
...
function back_order_info($back_id)
{
    ...
    $where = " and seller_id='".$_SESSION['selller_id']."' ";
    ///* 获取管理员信息 */
//    $admin_info = admin_info();
//
//    /* 如果管理员属于某个办事处，只列出这个办事处管辖的发货单 */
...
function return_order_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT o.order_id, o.order_sn, o.add_time, o.order_status, o.shipping_status, o.order_amount, o.money_paid," .
                ...
                " RIGHT JOIN " .$GLOBALS['ecs']->table('order_return'). " AS r ON r.order_id = o.order_id " .$where .
                " ORDER BY $filter[sort_by] $filter[sort_order] ".
                " LIMIT " . ($filter['page'] - 1) * $filter['page_size'] . ",$filter[page_size]";
    ...
    foreach ($row AS $key => $value)
    {
        ...
        $row[$key]['return_num'] = number_format($value['back'] + $value['exchange']); 
        if ($value['order_status'] == OS_INVALID || $value['order_status'] == OS_CANCELED)
        {
            /* 如果该订单为无效或取消则显示删除链接 */
            $row[$key]['can_remove'] = 1;
