 ...
 * ============================================================================
 * $Author: wang
 * $Id: category.php  2014-08-4 wang$
*/

define('IN_ECS', true);

require(dirname(__FILE__) . '/includes/init.php');
$exc = new exchange($ecs->table("seller_category"), $db, 'seller_cat_id', 'cat_name');

// 获得商家的店铺id
$store_id=$db->getOne("select id from ".$ecs->table('seller_shopinfo')." where seller_id='".$_SESSION['seller_id']."'");
...
{
    /* 获取分类列表 */
    $cat_list = seller_cat_list(0, 0, false);
...
if ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('cat_select',   seller_cat_list(0, 0, true));
...
if ($_REQUEST['act'] == 'insert')
{
	...
	$cat['is_text']      = !empty($_POST['is_text'])      ? intval($_POST['is_text'])    : 0;
    $cat['grade']        = !empty($_POST['grade'])        ? intval($_POST['grade'])      : 0;
    $cat['filter_attr']  = !empty($_POST['filter_attr'])  ? implode(',', array_unique(array_diff($_POST['filter_attr'],array(0)))) : 0;

    $cat['cat_recommend']  = !empty($_POST['cat_recommend'])  ? $_POST['cat_recommend'] : array();

    if (seller_cat_exists($cat['cat_name'], $cat['parent_id']))
    {
        /* 同级别下不能有重复的分类名称 */
	...
	if($_FILES['cat_img'])
	{
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                $file_dir ='../seller_imgs/seller_cat_img/seller_'.$_SESSION['seller_id'];
				if(!is_dir($file_dir))
				{
					mkdir($file_dir);	
				}
				$file_name=$file_dir."/cat_".gmtime().'.'.$ext;
                /* 判断是否上传成功 */
                if (move_upload_file($file['tmp_name'], $file_name))
                {
                    $cat['cat_img']=$file_name;
                }
                else
                {
					die();
	...
	$cat['seller_id']=$_SESSION['seller_id'];
    /* 入库的操作 */
    if ($db->autoExecute($ecs->table('seller_category'), $cat) !== false)
    {
        $cat_id = $db->insert_id();
        if($cat['show_in_nav'] == 1)
        {
			$count=$db->getOne("select count(id) from ".$ecs->table('seller_nav')." where nav_name='".$cat['cat_name']."'");
			if($count>0)
			{
				sys_msg('导航栏已存在同名导航', 0, $link);
			}
			else
			{
				$nav_order = $db->getOne("SELECT max(nav_order) FROM ". $ecs->table('seller_nav'));
				$nav_order += 2;
				$uri = build_uri('seller_goods', array('sid'=>$store_id,'cid'=> $cat_id), $cat['cat_name']);
				//显示在自定义导航栏中
				$sql = "INSERT INTO " . $ecs->table('seller_nav') .
					" (nav_name,nav_link,nav_order,is_show,is_blank,nav_img,is_text,seller_id,s_cid)".
					" VALUES('" . $cat['cat_name'] . "', '$uri', ".$nav_order.",'1','0','".$cat['cat_img']."',1,".$_SESSION['seller_id'].",'$cat_id')";
				$db->query($sql);
			}
...
if ($_REQUEST['act'] == 'edit')
{
    ...
    $smarty->assign('cat_select',  seller_cat_list(0, $cat_info['parent_id'], true));
    $smarty->assign('goods_type_list',  goods_type_list(0)); // 取得商品类型
...
elseif($_REQUEST['act'] == 'add_category')
{
    ...
    {
        $sql = "INSERT INTO " . $ecs->table('seller_category') . "(cat_name, parent_id, is_show)" .
...
if ($_REQUEST['act'] == 'update')
{
    ...
    {
        if (seller_cat_exists($cat['cat_name'],$cat['parent_id'], $cat_id))
        {
           $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
           sys_msg($_LANG['catname_exist'], 0, $link);
        }
    ...
    /* 判断上级目录是否合法 */
    $children = array_keys(seller_cat_list($cat_id, 0, false));     // 获得当前分类的所有下级分类
    ...
    $dat = $db->getRow("SELECT cat_name, show_in_nav,cat_img FROM ". $ecs->table('seller_category') . " WHERE seller_cat_id = '$cat_id'");
	...
	if($_FILES['cat_img'])
	{
        ...
        if ((isset($file['error']) && $file['error'] == 0) || (!isset($file['error']) && $file['tmp_name'] != 'none'))
        {
            ...
            else
            {
                ...
                {
                    $cat['cat_img']=$file_name;
					if($dat['cat_img']&&file_exists($dat['cat_img']))
					{
						@unlink($dat['cat_img']);	
					}
    ...
    if ($db->autoExecute($ecs->table('seller_category'), $cat, 'UPDATE', "seller_cat_id='$cat_id' and seller_id='".$_SESSION['seller_id']."'"))
    {
        if($cat['cat_name'] != $dat['cat_name'])
        {
            //如果分类名称发生了改变
            $sql = "UPDATE " . $ecs->table('seller_nav') . " SET nav_name = '" . $cat['cat_name'] . "' WHERE seller_id = '".$_SESSION['seller_id']."' AND s_cid = '" . $cat_id . "'";
            $db->query($sql);
        }
        if($cat['show_in_nav'] != $dat['show_in_nav'])
        {
            ...
            {
                //显示
                $nid = $db->getOne("SELECT id FROM ". $ecs->table('seller_nav') . " WHERE seller_id = '".$_SESSION['seller_id']."' AND s_cid = '" . $cat_id . "'");
                if(empty($nid))
                {
                    //不存在
                    $nav_order = $db->getOne("SELECT max(nav_order) FROM ". $ecs->table('seller_nav'));
                    ...
                    $sql = "INSERT INTO " . $ecs->table('seller_nav')." (nav_name,nav_link,nav_order,is_show,is_blank,nav_img,is_text,seller_id,s_cid)".
					" VALUES('" . $cat['cat_name'] . "', '$uri', ".$nav_order.",'1','0','".$cat['cat_img']."',1,".$_SESSION['seller_id'].",'$cat_id')";
					
                }
                else
                {
                    $sql = "UPDATE " . $ecs->table('seller_nav') . " SET is_show = 1 WHERE  seller_id = '".$_SESSION['seller_id']."' AND s_cid = '".$cat_id."'";
                }
                $db->query($sql);
            }
            else
            {
                //去除
                $db->query("UPDATE " . $ecs->table('seller_nav') . " SET is_show = 0 WHERE seller_id = '".$_SESSION['seller_id']."' AND s_cid = '".$cat_id."'");
            }
        }
        ...
        /* 更新分类信息成功 */
        clear_cache_files(); // 清除缓存
        admin_log($_POST['cat_name'], 'edit', 'seller_category'); // 记录管理员操作
...
if ($_REQUEST['act'] == 'move')
{
    ...
    $smarty->assign('cat_select', seller_cat_list(0, $cat_id, true));
...
if ($_REQUEST['act'] == 'move_cat')
{
    ...
    /* 更新商品分类 */
    $sql = "UPDATE " .$ecs->table('goods'). " SET seller_cat_id = '$target_cat_id' ".
...
if ($_REQUEST['act'] == 'toggle_show_in_nav')
{
    ...
    if (cat_update($id, array('show_in_nav' => $val)) != false)
    {
        ...
        {
            //显示
            $nav_order = $db->getOne("SELECT max(nav_order) FROM ". $ecs->table('seller_nav') . " WHERE seller_id='".$_SESSION['seller_id']."'");
            $nav_order += 2;
            $catname = $db->getOne("SELECT cat_name FROM ". $ecs->table('seller_category') . " WHERE seller_cat_id = '$id'");
            //显示在自定义导航栏中
            $_CFG['rewrite'] = 0;
            $uri = build_uri('seller_goods', array('sid'=>$store_id,'cid'=> $id), $cat['cat_name']);

            $nid = $db->getOne("SELECT id FROM ". $ecs->table('seller_nav') . " WHERE s_cid = '" . $id . "' AND seller_id = '".$_SESSION['seller_id']."'");
            if(empty($nid))
            {
                //不存在
                $sql = "INSERT INTO " . $ecs->table('seller_nav') . "(nav_name,nav_link,nav_order,is_show,is_blank,nav_img,is_text,seller_id,s_cid) VALUES('$catname', '$uri', ".$nav_order.",'1','0','',1,".$_SESSION['seller_id'].",'$id')";
            }
            else
            {
                $sql = "UPDATE " . $ecs->table('seller_nav') . " SET is_show = 1 WHERE s_cid = '" . $id . "' AND seller_id = '".$_SESSION['seller_id']."'";
            }
            $db->query($sql);
        }
        else
        {
            //去除
            $db->query("UPDATE " . $ecs->table('seller_nav') . "SET is_show = 0 WHERE s_cid = '" . $id . "' AND seller_id = '".$_SESSION['seller_id']."'");
        }
        clear_cache_files();
        make_json_result($val);
    }
...
if ($_REQUEST['act'] == 'remove')
{
    ...
    $cat_name = $db->getOne('SELECT cat_name FROM ' .$ecs->table('seller_category'). " WHERE seller_cat_id='$cat_id'");

    /* 当前分类下是否有子分类 */
    $cat_count = $db->getOne('SELECT COUNT(*) FROM ' .$ecs->table('seller_category'). " WHERE parent_id='$cat_id'");

    /* 当前分类下是否存在商品 */
    $goods_count = $db->getOne('SELECT COUNT(*) FROM ' .$ecs->table('goods'). " WHERE seller_cat_id='$cat_id'");

    /* 如果不存在下级子分类和商品，则删除之 */
    if ($cat_count == 0 && $goods_count == 0)
    {
        /* 删除分类 */
        $sql = 'DELETE FROM ' .$ecs->table('seller_category'). " WHERE seller_cat_id = '$cat_id'";
        if ($db->query($sql))
        {
            $db->query("DELETE FROM " . $ecs->table('seller_nav') . "WHERE s_cid = '" . $id . "' AND seller_id = '".$_SESSION['seller_id']."'");
            clear_cache_files();
...
// * @return  boolean
// */
function seller_cat_exists($cat_name, $parent_cat, $exclude = 0)
{
    $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('seller_category').
    " WHERE parent_id = '$parent_cat' AND cat_name = '$cat_name' AND seller_cat_id<>'$exclude' and seller_id='$_SESSION[seller_id]'";
...
{
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('seller_category'). " WHERE seller_cat_id='$cat_id' LIMIT 1";
...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_category'), $args, 'update', "seller_cat_id='$cat_id'");
}
...
function get_attr_list()
{
    $sql = "SELECT a.attr_id, a.cat_id, a.attr_name ".
           ...
           " WHERE  a.cat_id = c.cat_id AND c.enabled = 1 and a.seller_id='$_SESSION[seller_id]'".
