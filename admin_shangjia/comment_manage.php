...
if ($_REQUEST['act']=='reply')
{
    ...
    /* 获取评论详细信息并进行字符处理 */
    $sql = "SELECT * FROM " .$ecs->table('comment'). " WHERE comment_id = '$_REQUEST[id]' and seller_id='".$_SESSION['seller_id']."' ";
    ...
    /* 获得评论回复内容 */
    $sql = "SELECT * FROM ".$ecs->table('comment'). " WHERE parent_id = '$_REQUEST[id]' and seller_id='".$_SESSION['seller_id']."' ";
...
if ($_REQUEST['act']=='action')
{
    ...
    if (!empty($reply_info['content']))
    {
        ...
        $sql = "UPDATE ".$ecs->table('comment')." SET ".
               ...
               "ip_address= '$ip', ".
               "status    = 0".
               " WHERE comment_id = '".$reply_info['comment_id']."'  and seller_id='".$_SESSION['seller_id']."' ";
    }
    else
    {
        /* 插入回复的评论内容 */
        $sql = "INSERT INTO ".$ecs->table('comment')." (comment_type, id_value, email, user_name , ".
                    "content, add_time, ip_address, status, parent_id,seller_id) ".
               "VALUES('$_POST[comment_type]', '$_POST[id_value]','$_POST[email]', " .
                    "'$_SESSION[admin_name]','$_POST[content]','" . gmtime() . "', '$ip', '0', '$_POST[comment_id]',".$_SESSION['seller_id'].")";
    }
    $db->query($sql);

    /* 更新当前的评论状态为已回复并且可以显示此条评论 */
    $sql = "UPDATE " .$ecs->table('comment'). " SET status = 1 WHERE comment_id = '$_POST[comment_id]' and seller_id='".$_SESSION['seller_id']."' ";
    $db->query($sql);
    ...
    if (!empty($_POST['send_email_notice']) or isset($_POST['remail']))
    {
        ...
        $sql = 'SELECT user_name, email, content ' .
               ...
               " WHERE comment_id ='$_REQUEST[comment_id]' and seller_id='".$_SESSION['seller_id']."' ";
        $comment_info = $db->getRow($sql);
...
if ($_REQUEST['act'] == 'check')
{
    ...
    {
        /* 允许评论显示 */
        $sql = "UPDATE " .$ecs->table('comment'). " SET status = 1 WHERE comment_id = '$_REQUEST[id]' and seller_id='".$_SESSION['seller_id']."' ";
        $db->query($sql);
		
		$sql = 'SELECT id_value FROM '.$ecs->table('comment')." WHERE comment_id = '$_REQUEST[id]' and seller_id='".$_SESSION['seller_id']."' ";
		$goods_id = $db->getOne($sql);

		$sql = "SELECT COUNT(*) FROM ".$ecs->table('comment')." WHERE id_value = '$goods_id' AND comment_type = 0 AND status = 1 AND parent_id = 0  and seller_id='".$_SESSION['seller_id']."' ";	
		$count = $db->getOne($sql);
	
	
		$sql = "UPDATE ".$ecs->table('goods'). " SET comments_number = '$count' WHERE goods_id = '$goods_id' and seller_id='".$_SESSION['seller_id']."' ";
    ...
    {
        /* 禁止评论显示 */
        $sql = "UPDATE " .$ecs->table('comment'). " SET status = 0 WHERE comment_id = '$_REQUEST[id]' and seller_id='".$_SESSION['seller_id']."' ";
        $db->query($sql);
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $sql = "DELETE FROM " .$ecs->table('comment'). " WHERE comment_id = '$id' and seller_id='".$_SESSION['seller_id']."' ";
    $res = $db->query($sql);
    if ($res)
    {
        $db->query("DELETE FROM " .$ecs->table('comment'). " WHERE parent_id = '$id' and seller_id='".$_SESSION['seller_id']."' ");
    }
...
if ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        {
            case 'remove':
                $db->query("DELETE FROM " . $ecs->table('comment') . " WHERE  seller_id='".$_SESSION['seller_id']."' and " . db_create_in($_POST['checkboxes'], 'comment_id'));
                $db->query("DELETE FROM " . $ecs->table('comment') . " WHERE  seller_id='".$_SESSION['seller_id']."' and " . db_create_in($_POST['checkboxes'], 'parent_id'));
                break;

           case 'allow' :
               $db->query("UPDATE " . $ecs->table('comment') . " SET status = 1  WHERE  seller_id='".$_SESSION['seller_id']."' and " . db_create_in($_POST['checkboxes'], 'comment_id'));
               break;

           case 'deny' :
               $db->query("UPDATE " . $ecs->table('comment') . " SET status = 0  WHERE  seller_id='".$_SESSION['seller_id']."' and " . db_create_in($_POST['checkboxes'], 'comment_id'));
               break;
...
function get_comment_list()
{
    ...
    $sql = "SELECT count(*) FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 and seller_id='".$_SESSION['seller_id']."' $where";
    ...
    /* 获取评论数据 */
    $arr = array();
    $sql  = "SELECT * FROM " .$GLOBALS['ecs']->table('comment'). " WHERE parent_id = 0 and seller_id='".$_SESSION['seller_id']."' $where " .
