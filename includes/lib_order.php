...
function order_fee($order, $goods, $consignee,$order_counts=1)
{
	...
	$total['shipping_totelfee_formated']    = price_format($total['shipping_fee']*$order_counts, false);//青蜂网络www.0769web.net 商家入驻
...
function order_query_sql($type = 'finished', $alias = '')
{
    ...
    if ($type == 'finished') {
		...
		" AND {$alias}shipping_status " . db_create_in(array(SS_RECEIVED)) .//HZH
