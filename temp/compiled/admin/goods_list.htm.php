...
?>
  <tr>
	...
	<td align="center"><?php if ($this->_var['goods']['check_status'] == 0): ?>未审核<?php elseif ($this->_var['goods']['check_status'] == 1): ?>审核通过<?php else: ?><span title="<?php echo $this->_var['goods']['check_cause']; ?>">审核不通过</span><?php endif; ?></td>
    <?php endif; ?>
    <td align="center">
      ...
      <a href="goods.php?act=edit&goods_id=<?php echo $this->_var['goods']['goods_id']; ?><?php if ($this->_var['code'] != 'real_goods'): ?>&extension_code=<?php echo $this->_var['code']; ?><?php endif; ?>" title="<?php echo $this->_var['lang']['edit']; ?>" target="_blank"><img src="images/icon_edit.gif" width="21" height="21" border="0" /></a>
...
<div>
  ...
  <select name="type" id="selAction" onchange="changeAction()">
    ...
    <option value="move_to"><?php echo $this->_var['lang']['move_to']; ?></option>
	 <!--青蜂网络www.0769web.net 商家入驻 start-->
    <?php if ($this->_var['check_action']): ?>    
   <option value="check_goods">审核</option>
    <?php endif; ?>
    <!--青蜂网络www.0769web.net 商家入驻 end-->
  ...
  <!--青蜂网络www.0769web.net 商家入驻 start-->
  <?php if ($this->_var['check_action']): ?>
  <select name="goods_check" style="display:none" onchange="check_message()">
      <option value="0">未审核</option>
      <option value="1">审核通过</option>
      <option value="2">审核不通过</option>
  </select>
  <input  type="input" name="check_cause" style="display:none" value="未通过原因"  onblur="if(this.value=='未通过原因')this.value=''" />
  <?php endif; ?>
  <!--青蜂网络www.0769web.net 商家入驻 end-->
	<?php if ($this->_var['suppliers_list'] > 0): ?>
  <!--二级主菜单：转移供货商-->
...
//青蜂网络www.0769web.net 商家入驻
  function check_message(){
      var frm = document.forms['listForm'];
      
      if( frm.elements['goods_check'].value == 2){

           frm.elements['check_cause'].style.display ='';  
          
      }
      else{
          frm.elements['check_cause'].style.display ='none';
      }
  }
  function changeAction()
  {
      var frm = document.forms['listForm'];
	  frm.elements['goods_check'].style.display = frm.elements['type'].value == 'check_goods' ? '' : 'none'; //青蜂网络www.0769web.net 商家入驻
      // 切换分类列表的显示
