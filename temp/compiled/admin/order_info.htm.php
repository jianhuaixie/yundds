...
<table width="100%" cellpadding="3" cellspacing="1">
  ...
  <tr>
    ...
    <td><?php if ($this->_var['exist_real_goods']): ?><?php if ($this->_var['order']['shipping_id'] > 0): ?>
		<select id="shipping_name">
		<?php $_from = $this->_var['postcom']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'v');if (count($_from)):
    foreach ($_from AS $this->_var['v']):
?>
		<option value="<?php echo $this->_var['v']['name']; ?>" <?php if ($this->_var['v']['name'] == $this->_var['order']['shipping_name']): ?> selected <?php else: ?><?php endif; ?>><?php echo $this->_var['v']['name']; ?></option>
		<?php endforeach; endif; unset($_from); ?><?php $this->pop_vars();; ?>
		</select>
	<!--<span id="shipping_name"><?php echo $this->_var['order']['shipping_name']; ?></span><?php else: ?><?php echo $this->_var['lang']['require_field']; ?><?php endif; ?><a href="javascript:void(0);" class="special"><?php echo $this->_var['lang']['edit']; ?></a>-->&nbsp;&nbsp;<input type="button" onclick="window.open('order.php?act=info&order_id=<?php echo $this->_var['order']['order_id']; ?>&shipping_print=1')" class="button" value="<?php echo $this->_var['lang']['print_shipping']; ?>"> <?php if ($this->_var['order']['insure_fee'] > 0): ?>（<?php echo $this->_var['lang']['label_insure_fee']; ?><?php echo $this->_var['order']['formated_insure_fee']; ?>）<?php endif; ?><?php endif; ?></td>
  ...
  <tr>
    ...
    <td><?php if ($this->_var['order']['shipping_id'] > 0 && $this->_var['order']['shipping_status'] > 0): ?><span><?php if ($this->_var['order']['invoice_no']): ?><span id="invoice_no"><?php echo $this->_var['order']['invoice_no']; ?></span><?php else: ?><span id="invoice_no">N/A<?php endif; ?></span><a href="order.php?act=edit&order_id=<?php echo $this->_var['order']['order_id']; ?>&step=shipping" class="special"><?php echo $this->_var['lang']['edit']; ?></a><?php endif; ?></td>
...
<table cellpadding="3" cellspacing="1">
  ...
  <tr>
    ...
    <td colspan="5">
	<!-- 一键发货 -->      
		<?php if ($this->_var['order']['pay_status'] == 2 && $this->_var['order']['shipping_status'] != 1 && $this->_var['order']['order_status'] == 1): ?><input name="invoice_no" type="text" style="color:#00F" value="" /><input name="to_shipping" type="submit" value="快速发货" class="button"/><?php endif; ?>
...
<h5><span>物流跟踪(如果未填单号，下方将显示“正在查询物流信息，请稍后...”）</span></h5>
<div class="blank"></div>
...
$("#shipping_name").change(function(){
	...
	$order_sn = "<?php echo $this->_var['order']['order_sn']; ?>";

	Ajax.call('order.php?order_ajax=1', 'shipping_name='+$shipping_name+"&order_sn="+$order_sn, response_goods_info , 'POST', 'TEXT');
	function response_goods_info(data){
...
<script language="javascript">
  ...
  var expressid = document.getElementById("shipping_name").value;
