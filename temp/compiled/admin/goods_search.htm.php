  ...
  <form action="javascript:searchGoods()" name="searchForm">
    ...
    <select name="intro_type"><option value="0"><?php echo $this->_var['lang']['intro_type']; ?></option><?php echo $this->html_options(array('options'=>$this->_var['intro_list'],'selected'=>$_GET['intro_type'])); ?></select>
     <?php if ($this->_var['suppliers_exists'] == 1): ?>    
	  ...
	  <select name="status"><option value="3">选择审核状态</option><option value="0">未审核</option><option value="1">审核通过</option><option value="2">审核未通过</option></select> 
    <?php endif; ?>
    <!-- 关键字 -->
	<select name="seller_id"><option value="3">选择商家</option><option value="0">自营商品</option><option value="1">商家商品</option></select>
...
<script language="JavaScript">
    function searchGoods()
    {
		...
		listTable.filter['seller_id'] = document.forms['searchForm'].elements['seller_id'].value;
