...
<div class="list-div">
  ...
  <tr><td>
    <table>
      <tr>
        <td width="250" align="center"><img style=" border:3px solid #ccc; padding:5px;" id="screenshot" src="<?php echo $this->_var['curr_template']['screenshot']; ?>"/></td>
...
?>
  <div style="display:-moz-inline-stack;display:inline-block;vertical-align:top;zoom:1;*display:inline;">
    <table style="width: 220px;">
      <tr>
        <td><strong><a href="<?php echo $this->_var['template']['uri']; ?>" target="_blank" title="点击查看模板效果图"><?php echo $this->_var['template']['name']; ?></a></strong></td>
      </tr>
      <tr>
        <td><?php if ($this->_var['template']['screenshot']): ?><img src="<?php echo $this->_var['template']['screenshot']; ?>" style="cursor:pointer; float:left; margin:0 2px;display:block; border:3px solid #ccc; padding:5px;" id="<?php echo $this->_var['template']['code']; ?>" onclick="javascript:setupTemplate('<?php echo $this->_var['template']['code']; ?>')"/><?php endif; ?></td>
      </tr>
      <tr>
        <td valign="top">
...
function setupTemplate(tpl)
{
  ...
  if (confirm('启用新的Flash样式将覆盖原来的样式。\n您确定要启用选定的样式吗？'))
  {
    Ajax.call('seller_template.php?is_ajax=1&act=install', 'tpl_name=' + tpl + '&tpl_fg='+ StyleCode, setupTemplateResponse, 'GET', 'JSON');
  }
}
