...
<div class="tab-div">
    ...
    <div id="tabbar-div">
      <p>
        ...
        <span class="tab-back" id="packing-tab">包装清单</span>
    <span class="tab-back" id="customer-tab">售后保障</span>
      ...
      <form enctype="multipart/form-data" action="" method="post" name="theForm" >
        ...
        <table width="90%" id="general-table" align="center">
          ...
          <tr>
            ...
            <td>
            <select name="seller_cat_id"><option value="0"><?php echo $this->_var['lang']['select_please']; ?></option>
...
?>
        ...
        <table width="90%" id="packing-table" style="display:none">
        <tr>
          <td><?php echo $this->_var['FCKeditor2']; ?></td>
        </tr>
      </table>
      
      
      <table width="90%" id="customer-table" style="display:none">
        <tr>
          <td><?php echo $this->_var['FCKeditor3']; ?></td>
        </tr>
      </table>
...
<div id="feedbox" style="display:none;"><h2>选择属性图片<Span onClick="closeedBox('feedbox')">关闭</Span></h2><div id="feedcontent"></div></div>
<!--模板堂修改 end by zhouH-->

<!-- end goods form -->
<?php echo $this->smarty_insert_scripts(array('files'=>'validator.js,tab.js,showbox.js')); ?>
<script language="JavaScript">
  ...
  var integralPercent = <?php echo empty($this->_var['cfg']['integral_percent']) ? '0' : $this->_var['cfg']['integral_percent']; ?>;
	...
	/*模板堂修改 start by zhouH*/
	function show_goods_gallery(goodsId,this_obj)
	{
		img_id = this_obj.parentNode.childNodes[1].value ? this_obj.parentNode.childNodes[1].value : 0;

		Ajax.call('goods.php?is_ajax=1&act=get_goods_gallery', "goods_id=" + goodsId+"&img_id=" + img_id, goodsGalleryResult, "GET", "JSON");

		if(document.getElementById('galleryKey'))
		{
			document.getElementById('galleryKey').id='';
		}
		this_obj.parentNode.id='galleryKey';
	}
	
	function goodsGalleryResult(result)
	{
		document.getElementById('feedcontent').innerHTML = result.content;
		showBox('feedbox');
	}
	
	function gallery_on(this_obj,gallery_id)
	{
		...
		Ajax.call('goods.php?is_ajax=1&act=insert_gallery_attr', "gallery_id=" + gallery_id, galleryOnResult, "GET", "JSON");
...
?>
  ...
  function validate(goods_id)
  {
	  ...
	  if (document.forms['theForm'].elements['seller_cat_id'].value == 0)
      {
          validator.addErrorMsg('店内分类必须选择');
      }
...
?>
  ...
  /*wang 商家入驻
   *type 分类对象，0网站分类，1入驻商分类
   */
  function changecat(first_cat,child_cat,types)
  {
	 var type=types>0?types:0;
	 var firstcat=document.forms['theForm'].elements[first_cat];
	 var childcat=document.forms['theForm'].elements[child_cat];
	 
	 var first_id=firstcat.value;
	 childcat.style.display=first_id>0?'':'none';
	 if(first_id=='0')
	 {
		var cat_id=document.forms['theForm'].elements['cat_id'];
		cat_id.style.display='none';
		var seller_cat_id=document.forms['theForm'].elements['seller_cat_id'];
		seller_cat_id.style.display='none';
	 }
	 
	 var params='first_id='+first_id+'&type='+type+'&catobj='+child_cat;
	 
	 Ajax.call('goods.php?is_ajax=1&act=child_cat', params, addCatResponse, 'GET', 'JSON');
  }
  
  function addCatResponse(cate)
  {
	   if(!cate.error)
	   {
			var childcat=document.forms['theForm'].elements[cate.message];
			for (i = 0; i < cate.content.length; i ++ )
			{
			  var opt = document.createElement("OPTION");
			  opt.value = cate.content[i].cat_id;
			  opt.text  = cate.content[i].cat_name;
		
			  childcat.options.add(opt);
			}
	   }
  }
