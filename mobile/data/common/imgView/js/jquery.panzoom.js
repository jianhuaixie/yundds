...
(function(global, factory) {
	...
	if ( window.PointerEvent ) {
		...
		/**
		 * Support: Android
		 * Android sets pageX/Y to 0 for any touch event  设置pagex / Y 为 0任何触摸事件
		 * Attach first touch's pageX/pageY and clientX/clientY if not set correctly  如果不正确，固定第一次的 pageX/pageY and clientX/clientY 
		 */
		...
		$.each(list, function( i, name ) {
			// No equivalent touch events for over and out 超过和外时没有同等接触事件
			if (i < 2) {
	...
	/**
	 * Utility for determing transform matrix equality  确定变换矩阵等式的效用
	...
	/**
	 * Creates the options object for reset functions  创建重置函数的选项对象
	...
	/**
	 * Represent a transformation matrix with a 3x3 matrix for calculations  用于计算3x3矩阵表示的变换矩阵
	 * Matrix functions adapted from Louis Remi's jQuery.transform (https://github.com/louisremi/jquery.transform.js)
	 * @param {Array|Number} a An array of six values representing a 2d transformation matrix 表示二维变换矩阵的六值数组
	 */
	...
	Panzoom.prototype = {
		...
		/**
		 * Calculates the distance between two touch points  计算两点之间的距离
