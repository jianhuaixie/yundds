...
class ClipsBaseModel extends BaseModel {
    ...
    public function insert_user_account($surplus, $amount) {
		...
		//$data['tixian'] = abs($amount);//hzh
		$data['tixian'] = abs($surplus['amount2']);//hzh
		$data['payee'] = $surplus['payee'];//hzh
		$data['bank_name'] = $surplus['bank_name'];//hzh
		$data['bank_account'] = $surplus['bank_account'];//hzh
    ...
    public function get_user_tixian($user_id) {
        ...
        return $this->field('SUM(tixian)', $condition);
    ...
    public function get_user_default($user_id) {
        ...
        $sql = "SELECT pay_points, user_money, credit_line, last_login, is_validated, msn,avatar FROM " . $this->pre . "users WHERE user_id = '$user_id'";//hzh
        ...
        //$info['integral'] = $row['pay_points'] . C('integral_name');//hzh

		$info['integral'] = $row['pay_points'];//hzh

		$info['integral_name'] = C('integral_name');//hzh
		
		$info['nicheng'] = $row['msn'];//hzh
		$info['avatar'] = $row['avatar'];//hzh
		...
		//hzh  计算申请提现后数据
		$tixian_amount = $this->get_user_tixian($_SESSION ['user_id']);
		//$info['surplus2'] = $tixian_amount;//hzh 申请提现后立即显示的余额
		$info['surplus2'] = price_format($row['user_money'] - $tixian_amount, false);
		//hzh e
