...
<Nonce><![CDATA[%s]]></Nonce>
	...
	public function getUserInfo($openid){
		...
		$result = $this->http_get(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->get_token().'&openid='.$openid);
	...
	public function getOauthUserinfo($access_token,$openid,$type='0'){
	...
	public function get_token(){//shanmao.me add 

	//dump(S('access_tokens'));	

	if($this->getCache('access_tokens')) $this->getCache('access_tokens');

	

	$url = "https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=".$this->appid."&secret=".$this->appsecret;

        $ret_json = $this->http_get($url);
       ...
       // dump($ret);
		...
		$this->setCache('access_tokens',$ret->access_token,7200);

        //	setcookie('access_tokens',$ret -> access_token,time()+7200);

			//S('access_tokens',$ret -> access_token,7200);
