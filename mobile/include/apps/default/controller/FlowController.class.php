...
class FlowController extends CommonController {
    ...
    public function checkout() {
		//如果该用户没有绑定手机号码，需要绑定手机号码才能看到自己的推广中心
		$user_id=$_SESSION['user_id'];
		$mobile_phone = $this->model->table('users')->field('mobile_phone')->where('user_id='.$user_id)->getOne();
		if(empty($mobile_phone)){//如果手机号码为空，跳转到手机号码绑定
			show_message("需要绑定手机号码", "请绑定手机号码", url('user/bd_new'), 'checkout');
			die();
		}
        /* 取得购物类型 */
		...
		//hzh s
		$consignee['region_province'] = $this->model->table('region')->field('region_name')->where('region_id = ' . $consignee['province'])->getOne();
		$consignee['region_city'] = $this->model->table('region')->field('region_name')->where('region_id = ' . $consignee['city'])->getOne();
		$consignee['region_district'] = $this->model->table('region')->field('region_name')->where('region_id = ' . $consignee['district'])->getOne();
		$consignee['region'] = $consignee['region_province'] . $consignee['region_city'] . $consignee['region_district'] . ' ';
		//hzh e
        ...
        if ((!isset($use_surplus) || C('use_surplus') == '1') && $_SESSION ['user_id'] > 0 && $user_info ['user_money'] > 0) {
			...
			$tixian_amount = model('ClipsBase')->get_user_tixian($_SESSION ['user_id']);//提现申请后冻结的资金 hzh
			$new_surplus =$user_info ['user_money'] - $tixian_amount ;//扣除提现申请冻结资金后的可用余额 hzh
			
			$this->assign('your_surplus', $new_surplus);//hzh
        }
        ...
        if ((!isset($use_integral) || C('use_integral') == '1') && $_SESSION ['user_id'] > 0 && $user_info ['pay_points'] > 0 && ($flow_type != CART_GROUP_BUY_GOODS && $flow_type != CART_EXCHANGE_GOODS)) {
			...
			//hzh s
			if($user_info ['pay_points'] < model('Flow')->flow_available_points()){
				$can_use_point = $user_info ['pay_points'];
			}else{
				$can_use_point = model('Flow')->flow_available_points();
			}
			
			$total_goods_integral = $total['goods_price'] * 100;//hzh
			if($total_goods_integral < $can_use_point){
				$can_use_point = $total_goods_integral;
			}
			
			$this->assign('can_use_point', $can_use_point); // 用户可用积分 hzh
			$this->assign('your_integral_price', price_format($can_use_point / 100)); // 用户积分对应价格转化 hzh
			//hzh e
        }
    ...
    public function done() {
        ...
        if ($payment ['pay_code'] == 'balance' && $order ['order_amount'] > 0) {
			...
			if ($order ['order_amount'] > ($new_surplus + $user_info ['credit_line'])) {//冻结提现申请后可用部分 hzh
        ...
        if (method_exists('WechatController', 'do_oauth')) {
          ...
          // send_tx_msg("订单提醒","您的订单：".$order['order_sn']."已经成功生成！",$order_url); // 山猫的下单通知 
          //sendmb_order('订单提醒',"您的订单：".$order['order_sn']."已经成功生成！",$order['order_sn'],$order_url);
            //sendmb_order($title,$desc,$ordersn,$url,$openid=null)
            //sendmb_order("订单时间:".date('Y-m-d H:i:s',time()),"订单编号:".$order['order_sn'],"订单状态：待付款",$order_url);
            
            $order_id=$order ['order_id'];
            $order_sn=$order['order_sn'];
            $res_buyer_info=mysql_query("select consignee,address,postscript,mobile,province,city,district from ydcom_order_info where order_id=$order_id");
            while ($row_buyer_info=mysql_fetch_array($res_buyer_info)){
            	$buyer_name=$row_buyer_info[0];
            	$buyer_address=$row_buyer_info[1];
            	$postscript=$row_buyer_info[2];
				if(empty($postscript)){
				$postscript="无";
				}
            	$mobile=$row_buyer_info[3];
            	$province_id=$row_buyer_info[4];
            	$city_id=$row_buyer_info[5];
            	$district_id=$row_buyer_info[6];
            	$re1=mysql_query("select region_name from ydcom_region where region_id=$province_id");
            	while ($ro1=mysql_fetch_array($re1)){
            		$province_name=$ro1[0];
            	}
            	$re2=mysql_query("select region_name from ydcom_region where region_id=$city_id");
            	while ($ro2=mysql_fetch_array($re2)){
            		$city_name=$ro2[0];
            	}
            	$re3=mysql_query("select region_name from ydcom_region where region_id=$district_id");
            	while ($ro3=mysql_fetch_array($re3)){
            		$district_name=$ro3[0];
            	}
            ...
            $res_g_name=mysql_query("select goods_name,goods_price from ydcom_order_goods where order_id=$order_id");
            while ($row_g_name=mysql_fetch_array($res_g_name)){
            	$g_name=$row_g_name[0];
            	$g_price=$row_g_name[1];
            }
           // sendmb_neworder($order_sn,$order_goods,$order_amount, $order_buyer,$order_address,$order_note,$desc,$url,$openid=null);
            sendmb_neworder($order_sn,$g_name,$g_price, $buyer_name."  联系电话:".$mobile,"$province_name"."$city_name"."$district_name"."$buyer_address",$postscript,"该订单还未付款，请尽快支付！",$order_url,$openid=null);
        ...
        if ($order ['order_amount'] <= 0) {
            ...
            // sendmb_order('订单支付提醒',"您的订单：".$order['order_sn']."已经支付成功，我们会尽快安排发货！",$order['order_sn'],$order_url);
           //  send_tx_msg("订单支付提醒","您的订单：".$order['order_sn']."已经支付成功，我们会尽快安排发货！",$order_url); // shanmao.me  余额支付通知
            //sendmb_pay_done($title,$amount,$goods_name,$desc,$url,$openid=null);
            //sendmb_pay_done($title,$amount,$goods_name,$desc,$url,$openid=null);
            ...
            // $sqlhelper->updateuser_fenxiaorankand_rank_points($order_id);
             ...
             if ($rank_buyer_now>$rank_buyer_old){
             	...
             	//$user_rank_buyer_name=$sqlhelper->getuser_rank_name($user_id_buyer);
              	...
              	sendmb_level_change("恭喜您推广等级提升",$rank_buyer_old_name,$user_rank_buyer_name,$uname_buyer,$url,$openid_buyer);
             }
             //订单金额
             ...
             while($row0=mysql_fetch_array($res0)){
             	...
             	while($row02=mysql_fetch_array($res02)){
             		...
             		sendmb_pay_done("",$order_amount."元",$order_sn,'我们会尽快安排为您发货',$url,$openid0);
             	}
             }
             ...
             for(;$b1>0;){
             	...
             	if ($d1>=$i){
             		...
             		//sendmb_commission($title,$amount,$desc,$url,$openid=null)
             		sendmb_commission("您获得了一笔新的推广收益",$f1."元","订单编号：".$order_sn,$url,$openid1);
             ...
             for(;$b2>0;){
             	...
             	if ($d2>$mbb_z){	
             		...
             		//sendmb_order("推广通知","恭喜您获得了一笔新的渠道收益".$f2."元",$order_sn,$url,$openid);
        ...
        $this->assign('total', $total);
        $this->assign('goods_list', $cart_goods);
        $this->assign('order_submit_back', sprintf(L('order_submit_back'), '<a href="'.url('index/index').'&u='.$_SESSION['user_id'].'">返回首页</a>', L('goto_user_center'))); // 返回提示 hzh
