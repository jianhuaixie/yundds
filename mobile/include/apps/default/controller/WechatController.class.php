...
class WechatController extends CommonController
{
 ...
 $this->model->table('wxtext')
	...
	public function grabBonus(){
		...
		$openid=$user_id;
		//var_dump($openid);die();
		//判断是否在活动时间  上午10点   下午3点  晚上8点  在其他时间点，都是不能开始抢红包的
		$d=date('Y-m-d-H-i-s',time());
		$arr=explode('-',$d);
		if($arr[3]<10 or $arr[3]>24){
			die("活动未开始，敬请期待！");
		}
		$s0=implode($arr,'-');$time0=strtotime($s0);
		$arr[4]='00';$arr[5]='00';
		$arr[3]=2;$s1=implode($arr,'-');$start_time1=strtotime($s1);//当日上午10点开始的时间戳（活动开始时间）
		$arr[3]=7;$s2=implode($arr,'-');$start_time2=strtotime($s2);//当日下午3点开始的时间戳
		$arr[3]=12;$s3=implode($arr,'-');$start_time3=strtotime($s3);//当日晚上8点开始的时间戳
		$arr[3]=16;$s4=implode($arr,'-');$stop_time=strtotime($s4);//当日晚上12点结束的时间戳（活动结束时间）
		if(time()<$start_time2){
			$start_time=$start_time1;
		}elseif(time()<$start_time3){
			$start_time=$start_time2;
		}elseif(time()<$stop_time){
			$start_time=$start_time3;
		}
		//判断该用户是否已经领取过红包
		$grab_flag=model('Base')->model->table('wechat_grabbonus')->field('grab_flag')->order('grab_time desc')->where('openid='.$openid)->getOne();//最近期的grab_flag为0，已经领过红包了
		//如果grab_flag==0，判断上次抢红包的时间是否在本次活动时间内，如果是的，不能再抢了
		if($grab_flag=='0'){
			$grab_time=model('Base')->model->table('wechat_grabbonus')->field('grab_time')->order('grab_time desc')->where('openid='.$openid)->getOne();
			if($grab_time<$start_time){//是上一次活动抢的红包，将grab_flag的状态修改为1，又可以抢红包了
				$this->model->query("update ydcom_wechat_grabbonus set grab_flag = 1 where openid=".$openid);
			}else{
				echo '领过了';
				die();
			}
		...
		if(!empty($template) || !empty($bonus_msg)){
		 ...
		 //微信发红包 by  shanmao.me
		$allhb = model("Base")->model->table('touch_shop_config')->field('value')->where("code='allhb'")->getOne();
		$openid=model('Base')->model->table('wechat_user')->field('openid')->where('ect_uid='.$openid)->getOne();
		//var_dump(C("hbend"));die();  //配置文件里可以设置抢红包的活动日期
		if(C('openfhb') && $allhb>0 ){//红包开关,获取钱包中的数量，并且开放了抢红包，而且用户是存在的
			
			$hbmoney = rand(C("hbstart")*100,C("hbend")*100)/100;					
			if(strtotime(C("starttime"))<time() && strtotime(C("endtime"))>time() && $hbmoney>0 && $openid){			
				require(getcwd()."/wxhongbao.php");
				$hb = new Common_util_pub();
				$arr['openid']=$openid;
				$arr['hbname']="抢红包";
				$arr['body']="抢红包活动";
				$arr['fee']=$hbmoney;
				...
				if($re['result_code']=='SUCCESS'){
					//将剩余总金额减去红包的金额   ydcom_touch_shop_config  里面的value值来设定活动的总金额
					$data['gzhb']=	$hbmoney;
					$this->model->query("update ydcom_touch_shop_config set value = value-".$hbmoney." where code ='allhb'");
					//抢红包信息存入数据库
					$bonusData['openid']=$openid;
					$bonusData['grab_time']=time();
					$bonusData['grab_flag']=0;//已经领取红包
					$bonusData['start_time']=$start_time1;
					$bonusData['stop_time']=$start_time4;
					$bonusData['total_money']=200;
					$bonusData['bonus_num']=100;
					$bonusData['max_bonus']=101;
					$bonusData['max_bonus']=$hbmoney;
					$res=$this->model->table('wechat_grabbonus')
								->data($bonusData)
								->insert();
				}
				var_dump($re);die();				
			}else{
				echo '抢红包失败！可能原因：活动过期了！你未登陆！';
				die();
			}					
		}else{
			echo '红包抢光啦！';
    ...
    private function subscribe($openid = '', $scene_id = 0)
    {
        if(!empty($openid)){
            ...
            $rs = $this->model->table('wechat_user')
                ->field('ect_uid, subscribe,nickname')
                ->where($where)
                ->find();
            // 未关注
            if (empty($rs)) {
				...
				//微信发红包 by  shanmao.me
				$allhb = model("Base")->model->table('touch_shop_config')->field('value')->where("code='allhb'")->getOne();
				if(C('openfhb') && $allhb>0){//红包开关
					$hbmoney = rand(C("hbstart")*100,C("hbend")*100)/100;					
					if(strtotime(C("starttime"))<time() && strtotime(C("endtime"))>time() && $hbmoney>0){			
						require(getcwd()."/wxhongbao.php");
						$hb = new Common_util_pub();
						$arr['openid']=$openid;
        	$arr['hbname']="关注";
		...
		//微信发红包 by  shanmao.me
			...
			$datas['wx_nickname'] = $info['nickname'];
			$datas['wx_headimgurl'] = $info['headimgurl'];
			$datas['wx_p_nickname'] = $this->model->table('wechat_user')->field('nickname')->where("ect_uid = '$p_id'")->getOne();
			$datas['wx_p_headimgurl'] = $this->model->table('wechat_user')->field('headimgurl')->where("ect_uid = '$p_id'")->getOne();
			$this->model->table('users')
				->data($datas)
				->where('user_name = "' . $username . '"')
				->update();
			...
			$u_id = $_SESSION['user_id'];
		    $p_id = model('Base')->model->table('users')->field('parent_id')->where("user_id = '$u_id'")->getOne();//get parent_id
			//$reg_time = model('Base')->model->table('users')->field('reg_time')->where("user_id = '$u_id'")->getOne();//get parent_id
			//$r_time = local_date('Y-m-d H:i', $reg_time);
		    $p_openid = model('Base')->model->table('wechat_user')->field('openid')->where("ect_uid = '$p_id'")->getOne();
			$url =  __HOST__ . url('User/my_tuiguang');
			sendmb_user('您好，【YD'.$u_id.'】已通过扫描您的专属二维码被推荐成为会员。','','YD'.$u_id,$url,$p_openid);
			//sendmb_user('title2','desc','uname','url','oIS5fwiOc2GiMlOMl_BqDvDFF_pk');
			//hzh e
                ...
                //$data['attention_time'] = $info['attention_time'];
                $data['remark'] = $info['remark'];
                $data['unionid'] = isset($info['unionid']) ? $info['unionid'] : '';
                ...
                //微信发红包 by  shanmao.me
				$allhb = model("Base")->model->table('touch_shop_config')->field('value')->where("code='allhb'")->getOne();
				if(C('openfhb') && $allhb>0 && !$rs['nickname']){//红包开关
					$hbmoney = rand(C("hbstart")*100,C("hbend")*100)/100;					
					if(strtotime(C("starttime"))<time() && strtotime(C("endtime"))>time() && $hbmoney>0){			
						require(getcwd()."/wxhongbao.php");
						$hb = new Common_util_pub();
		...
		//微信发红包 by  shanmao.me
          ...
          //hzh s
          $u_id = model('Base')->model->table('wechat_user')->field('ect_uid')->where("openid = '$openid'")->getOne();
          //$attention_time = model('Base')->model->table('wechat_user')->field('attention_time')->where("user_id = '$u_id'")->getOne();
          $attention_time = time();
          $this->model->table('users')
          ->data('attention_time = "' . $attention_time . '"')
          ->where('user_id = "' . $u_id . '"')
          ->update();
          //hzh e
    ...
    static function update_weixin_user($userinfo, $wechat_id, $weObj)
    {
        ...
        if (empty($ret)) {
			...
			if($re['result_code']=='SUCCESS'){
				$data1['gzhb']=	$hbmoney;
				model('Base')->model->query("update ydcom_touch_shop_config set value = value-".$hbmoney." where code ='allhb'");			
			}			
			}					
			}*/
		//微信发红包 by  shanmao.me
		    ...
		    $p_openid = model('Base')->model->table('wechat_user')->field('openid')->where("ect_uid = '$p_id'")->getOne();
			$url =  __HOST__ . url('User/my_tuiguang');
			sendmb_user('您好，【YD'.$u_id.'】已通过您的专属链接被推荐成为会员。','','YD'.$u_id,$url,$p_openid);
			//hzh e
          ...
          //  var_dump($userinfo);
			...
			//$userinfo['remark']=time();
			 // logResult(var_export($userinfo, true)); 
			  
            model('Base')->model->table('wechat_user')
                ->data("headimgurl='".$userinfo['headimgurl']."',nickname='".$userinfo['nickname']."'")
                ->where('openid = "' . $userinfo['openid'] . '"')
                ->update();
