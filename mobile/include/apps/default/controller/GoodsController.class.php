...
// 			}
			...
			//hzh s
			$where['goods_id'] = $this->goods_id;
            $rs = $this->model->table('products')
                ->field('goods_id')
                ->where($where)
                ->find();
			if (!empty($rs)) {
					$goods_attr = implode('|',$attr_id);
					//$sql = "select product_number from " . $ecs->table("products") . " where goods_id = $goods_id and goods_attr = '$goods_attr'";
					//$attr_ku = $db->getOne($sql);
					//$goods_id = $this->goods_id;
					$attr_ku = $this->model->table('products')->field('product_number')->where("goods_id = " . $this->goods_id ." AND goods_attr = '$goods_attr'")->getOne();
					if($attr_ku > 0){
						$res['attr_ku'] = $attr_ku;
					}else{
						$res['attr_ku'] = 0;
				...
				$res['attr_ku'] = $goods['goods_number'];
			}
			//hzh e
