...
function insert_collection_num()
...
function insert_not_pay_order_list_num() {
	...
	$count1 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 1 AND shipping_status =0 AND pay_status = 0')->count();



	$count2 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 0 AND shipping_status =0 AND pay_status = 0')->count();
...
function insert_to_receive_order_list_num() {
	...
	$count1 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 5 AND shipping_status = 1 AND pay_status =2')->count();



	$count2 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 1 AND shipping_status = 3 AND pay_status =2')->count();



	$count3 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 1 AND shipping_status = 0 AND pay_status =2')->count();



	$count4 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 5 AND shipping_status = 5 AND pay_status =2')->count();
...
function insert_to_comment_order_list_num() {
	...
	$count = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 5 AND shipping_status = 2')->count();
...
function insert_to_return_order_list_num() {
	...
	$count1 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 4 AND shipping_status = 0 AND pay_status = 0')->count();
	$count2 = M()->table('order_info')->where('user_id = ' . $user_id . ' AND order_status = 4 AND shipping_status = 1 AND pay_status = 2')->count();
	$count = $count1 + $count2;
...
function insert_msg_num_all()

{

	$user_id = $_SESSION['user_id'];

	$count = M()->table('feedback ')

            ->field('count(*)')

            ->where(" user_id = '$user_id' AND parent_id = 0 AND user_name = '" . $_SESSION['user_name'] . "' AND order_id=0")
...
function insert_user_bonus_num()

{

	$user_id = $_SESSION['user_id'];

	$count = M()->table('user_bonus')

            ->field('count(*)')

            ->where(" user_id = '$user_id' AND used_time = 0 AND order_id = 0")
...
function insert_surplus_amount() {
	...
	$surplus_amount = model('ClipsBase')->get_user_surplus($user_id);
    ...
    foreach ($res AS $row) {
        ...
        switch ($row['media_type']) {
            ...
            case 0: // 图片广告
                ...
                $ads[] = "<a href='" . url('default/affiche/index', array('ad_id' => $row['ad_id'], 'uri' => urlencode($row["ad_link"]))) . "'><img src='$src' width='" . $row['ad_width'] . "' height='$row[ad_height]' border='0' /></a>";
