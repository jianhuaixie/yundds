...
class IndexModel extends CommonModel {
    ...
    public function goods_list($type = 'best', $limit = 10, $start = 0) {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.goods_number, g.market_price, g.shop_price AS org_price, g.promote_price, g.cat_id, ' . "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, " . "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, g.goods_img, RAND() AS rnd " . 'FROM ' . $this->pre . 'goods AS g ' . "LEFT JOIN " . $this->pre . "member_price AS mp " . "ON mp.goods_id = g.goods_id AND mp.user_rank = '$_SESSION[user_rank]' ";
        ...
        foreach ($result as $key => $vo) {
			...
			$goods[$key]['cat_id'] = $vo['cat_id'];//hzh
			
			$goods[$key]['goods_number'] = $vo['goods_number'];//hzh
			...
			//$goods[$key]['url'] = url('goods/index', array('id' => $vo['goods_id']));//hzh
			...
			if(!empty($user_id)){
            	...
            	$goods[$key]['url'] = url('goods/index', array('id' => $vo['goods_id'])) . '&u=' . $user_id;
    ...
    function get_promote_goods($cats = '') {
        ...
        //$num = model('Common')->get_library_number("recommend_promotion");//hzh
		$num = 4;//hzh
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, ' .
                ...
                "g.is_best, g.is_new, g.is_hot, g.is_promote, g.is_shipping, RAND() AS rnd " .//hzh
        ...
        $sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY rnd';//HZH
		//$sql .= $order_type == 0 ? ' ORDER BY g.sort_order, g.last_update DESC' : ' ORDER BY g.promote_price ASC';//HZH
        ...
        foreach ($result AS $idx => $row) {
			...
			$goods[$idx]['sales_count'] = model('GoodsBase')->get_sales_count($row['goods_id']);
			//hzh e
...
// hzh s
    function get_floor_goods_cat($cid = '') {
        ...
        $sql = 'SELECT g.goods_id, g.cat_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.brand_id, g.goods_number, ' .
...
// hzh s
    function get_floor_goods_rand($bid = '') {
        ...
        $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price, g.brand_id, g.goods_number, ' .
