...
class UsersModel extends BaseModel {
    ...
    function get_user_orders($user_id, $pay = 1, $num = 10, $start = 0) {
        ...
        if ($pay == 1) {
			...
			$pay ='and ((order_status =0 and shipping_status =0 and pay_status =0) or (order_status =1 and shipping_status =0 and pay_status =0))';

		} elseif ($pay == 3) {//待收货

			$pay ='and ((order_status =5 and shipping_status =1 and pay_status =2) or (order_status =1 and shipping_status =3 and pay_status =2) or (order_status =1 and shipping_status =0 and pay_status =2) or (order_status =5 and shipping_status =5 and pay_status =2))';

		} elseif ($pay == 4) {//待评价

			$pay ='and order_status =5 and shipping_status =2 and pay_status =2';

		} elseif ($pay == 5) {//退换货

			$pay ='and ((order_status =4 and shipping_status =0 and pay_status =0) or (order_status =4 and shipping_status =1 and pay_status =2))';
    ...
    function third_reg($info) {
        ...
        if ($this->register($username, $password, $email) !== false) {
            ...
            //$touch_data['user_id'] = $uid;//hzh
			$touch_data['user_id'] = $_SESSION['user_id'];

            $touch_data['aite_id'] = $info['aite_id'];

            //$this->insert($touch_data);//hzh
			
			//hzh s do not insert database if exist
			$where['aite_id'] = $touch_data['aite_id'];
			$aite_id = $this->model->table('touch_user_info')->field('aite_id')->where($where)->getOne();
			if (!$aite_id) {
			...
			//发送上家信息 hzh s
			//$touch_data['msn'] = mt_rand(1000, 9999);
			$id = $_SESSION['user_id'];

			if($info['figureurl_qq_2']&&$info['nickname']){
				$sql ="update " . $this->pre . "users set msn ='".$info['nickname']."',avatar ='".$info['figureurl_qq_2']."' where user_id='$id' ";
				$this->query($sql);
			}
			if($info['avatar_hd']&&$info['screen_name']){
				$sql ="update " . $this->pre . "users set msn ='".$info['screen_name']."',avatar ='".$info['avatar_hd']."' where user_id='$id' ";
				$this->query($sql);
			}
			
			
			$p_id = $this->model->table('users')->field('parent_id')->where("user_id = '$id'")->getOne();//get parent_id
			$u_name = $this->model->table('users')->field('user_name')->where("user_id = '$id'")->getOne();//get this nicheng 
			$p_openid = $this->model->table('wechat_user')->field('openid')->where("ect_uid = '$p_id'")->getOne();
	
			$msg_title = "推广提醒";
			$msg_content = "新用户（".$u_name."）慕名而来，成为了您的忠实粉丝。";
			$msg_url = "";
			$msg_openid = $p_openid;
					
			send_tx_msg($msg_title, $msg_content, $msg_url, $msg_openid);
			//send_tx_msg($title,$msg,$url,$openid='');
			//hzh e
