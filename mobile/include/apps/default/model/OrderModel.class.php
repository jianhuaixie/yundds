...
class OrderModel extends BaseModel {
    ...
    function cart_goods($type = CART_GENERAL_GOODS) {
        ...
        foreach ($arr as $key => $value) {
			...
			$goods_thumb = $this->model->table('goods')->field('goods_thumb')->where("goods_id = '" . $arr[$key]['goods_id'] . "' ")->getOne();
    ...
    function last_shipping_and_payment() {
        ...
        if (empty($row)) {
            ...
            $row = array('shipping_id' => 1, 'pay_id' => 1);//hzh 0
        ...
        if (!isset($order['shipping_id']) || !isset($order['pay_id'])) {
            ...
            if ($_SESSION['user_id'] > 0) {
                ...
                if (!isset($order['shipping_id'])) {
                    ...
                    $order['shipping_id'] = 1;//hzh 0

                }

                if (!isset($order['pay_id'])) {

                    $order['pay_id'] = 1;//hzh 0
...
//    }
    ...
    function aftermarket_goods($rec_id) {
        ...
        $sql = "SELECT rg.rec_id, rg.goods_id, rg.goods_name, rg.goods_sn," .
                ...
                " LEFT JOIN " . $this->pre . "order_goods og ON rg.rec_id = og.rec_id  WHERE rg.rec_id = '$rec_id'";
        ...
        if ($res['extension_code'] == 'package_buy') {
            ...
            $res['package_goods_list'] = model('PackageBase')->get_package_goods($res['goods_id']);
    ...
    public function return_action($ret_id, $return_status = 0, $refund_status = 0, $is_check = 0, $note = '', $username = null, $place = 0, $action_info = null) {
        ...
        $sql = 'INSERT INTO ' . $this->pre .
                ...
                'return_action (ret_id, action_user, return_status, refund_status, is_check, action_place, action_note, log_time,action_info) ' .

                'SELECT ' .

                "ret_id, '$username', '$return_status', '$refund_status', '$is_check', '$place', '$note', '" . gmtime() . "' ,'" . $action_info . "' " .

                'FROM ' . $this->pre . "order_return WHERE ret_id = '$ret_id'";
    ...
    public function get_return_action($ret_id) {
        ...
        foreach ($res as $row) {
            ...
            $row['return_desc'] = sprintf(L('rd.' . $row['return_status']), $claim['service_name'], L('detail_shipping_status'), $order['shipping_status'], $reason['cause_name'], $order_return['should_return']);

            $row['refund_status'] = L('ff.' . $row['refund_status']);
    ...
    public function get_return_operate($order) {
        ...
        if ($order['is_check'] == RC_APPLY_FALSE && $order['return_status'] == RF_APPLICATION) {
            ...
            /* 仅退款 */ elseif ($server['service_type'] == ST_REFUND) {
            ...
            /* 维修 */ elseif ($server['service_type'] == ST_REPAIR) {
     ...
     * @param   int $ret_id 订单id（如果order_id > 0 就按id查，否则按sn查）
    ...
    function aftermarket_info($ret_id) {
        ...
        $sql = "SELECT r.*, t.service_name FROM " . $this->pre . "order_return r LEFT JOIN " . $this->pre .
                ...
                "service_type t ON r.service_id = t.service_id WHERE ret_id = '$ret_id'";
