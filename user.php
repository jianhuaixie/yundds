...
elseif ($action == 'order_detail')
{
    ...
    foreach ($goods_list AS $key => $value)
    {
        ...
        /*退换货 start*/
        $goods_list[$key]['ret_id'] = $db->getOne("SELECT ret_id FROM " .$ecs->table('order_return'). " WHERE rec_id='$value[rec_id]'");
        $goods_list[$key]['aftermarket'] = check_aftermarket($value['rec_id']); //查询是否申请过售后服务
...
elseif ($action == 'act_add_booking')
{
    ...
    $booking = array(
        ...
        //青蜂网络0769web.net 商家入驻
		'booking_id'   => isset($_POST['rec_id'])  ? intval($_POST['rec_id']) : 0,
		'seller_id' =>isset($_POST['s_id'])  ? intval($_POST['s_id']) : 0
    );
 ...
 * 退换货申请类型
 */
elseif( $action == 'aftermarket_type'){
    ...
    /*检查是否申请过售后服务，防止重复提交*/
    if (check_aftermarket($rec_id)) {
        show_message($_LANG['aftermarket_apply_error']);
    }
    $order = get_order_detail($order_id, $user_id);
  
    /*获得时间范围内可以提供的售后服务*/
    $type_list = get_service_opt($order); 
    $service_type = array();
   
    //查询所对应的可用服务类型
    if (!empty($type_list)) {
        $service_type = get_service_type_list($type_list);
    }
    if (empty($service_type)) {
        //没有任何售后服务可选  返回
        show_message($_LANG['no_service']);
    }
    ...
    $smarty->assign('order', $order);
    $smarty->assign('goods', order_goods_info($rec_id));
    $smarty->display('user_transaction.dwt');
}
/**
 * 退换货申请
 */
elseif( $action == "aftermarket_apply"){
    ...
    if ($service_type['service_type'] == ST_RETURN_GOODS) {
        /* 退货退款 */
        $refund_goods = true;
    } elseif ($service_type['service_type'] == ST_EXCHANGE) {
        /* 换货 */
        $exchange = true;
    }
    $order = get_order_detail($order_id, $user_id);
    $goods_info = order_goods_info($rec_id);
    ...
    $province_list[$region_id] = get_regions(1, $order['country']);
    $city_list[$region_id] = get_regions(2, $order['province']);
    $district_list[$region_id] = get_regions(3, $order['city']);
    $smarty->assign('shop_country',     $_CFG['shop_country']);
    $smarty->assign('shop_province',    get_regions(1, $_CFG['shop_country']));
    $smarty->assign('province_list',    $province_list);
    $smarty->assign('address',          $address_id);
    $smarty->assign('city_list',        $city_list);
    $smarty->assign('district_list',    $district_list);
    $smarty->assign('business_address', get_business_address($goods_info['suppliers_id']));
    $smarty->assign('rec_id', $rec_id);
    $smarty->assign('refund_goods', $refund_goods);
    $smarty->assign('exchange', $exchange);
    $smarty->assign('service_type_list', get_service_type_list());
    $smarty->assign('service_type', $service_type);
    $smarty->assign('cause_list', get_parent_cause()); //退换货原因
...
elseif ($action == 'upload_file') {
    ...
    $result   = array('error' => 0, 'content' => '');
    if ($_REQUEST["name"]) {
        $img_name = basename($image->upload_image($_FILES['file'],'service_image'));
        if ($result['error'] > 0) {
            die($json->encode($result)); 
        }
        /* 生成logo链接 保存图片到数据库/aftermarket_attachments*/
        $img['img_url'] = $ecs->url() . './data/service_image/' . $img_name;
        $img['goods_id'] = intval($_GET['goods_id']);
        $img['rec_id'] = intval($_GET['rec_id']);  
        $db->autoExecute($ecs->table('aftermarket_attachments'), $img, 'INSERT');
 ...
 * 提交退换货申请
 */
elseif( $action == 'aftermarket_done' ){
    include_once(ROOT_PATH . 'includes/lib_order.php');
    ...
    $num = check_aftermarket($rec_id);
    if ($num) {
        show_message($_LANG['aftermarket_apply_error']);
    }
    $goods = order_goods_info($rec_id); //订单商品
   
    $service_type = get_service_type(intval($_POST['service_id'])); //查询服务类型
    $reason = get_service_cause_name($_POST['reason']); //服务原因
    $order_return = array(
        ...
        'service_id' => intval($_POST['service_id']), //服务类型
        'user_id' => $user_id,
        'addressee' => $_POST['applyUserName'],  //联系人
        'phone' => $_POST['applyPhone'],    //联系方式
        'remark' => empty($_REQUEST['description']) ? '' : $_REQUEST['description'], //退款说明
        'should_return' => 0, //应退金额
        'cause_id' => intval($_POST['reason']), //退换货原因
        'add_time' => gmtime(),
        'return_status' => RF_APPLICATION,
        'refund_status' => FF_NOREFUND
    );
    /*换货*/
    if($service_type['service_type'] == ST_EXCHANGE){
        $order_return['country'] = 1;
        $order_return['province'] = isset($_POST['province']) ? intval($_POST['province']) : 0;
        $order_return['city'] = isset($_POST['city']) ? intval($_POST['city']) : 0;
        $order_return['district'] = isset($_POST['district']) ? intval($_POST['district']) : 0;
        $order_return['address'] = isset($_POST['address']) ? compile_str(trim($_POST['address'])) : '';
        $order_return['should_return'] = intval($_POST['back_num'])* $goods['goods_price'];//应退金额
    }
    /* 插入服务单表 */
    if ($query = $db->autoExecute($ecs->table('order_return'), $order_return, 'INSERT')) {
        $ret_id = $db->insert_id();

        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('return_goods') . " (rec_id, goods_id, goods_name,goods_sn,return_type,back_num,goods_attr,goods_price)" .
                "VALUES ('$rec_id', '$goods[goods_id]', '$goods[goods_name]','$goods[goods_sn]','$_POST[service_id]', '$_POST[back_num]', '$goods[goods_attr]','$goods[goods_price]')";
        $db->query($sql);
        $action_info = $_LANG['return_info']; 
        /* 记录log */
        return_action($ret_id, RF_APPLICATION,  FF_NOREFUND, RC_APPLY_FALSE, $order_return['remark'], $_LANG['buyer'], '', $action_info);  
        show_service_message('申请已提交', '', 'user.php?act=aftermarket_list', 'info');
        
    } else {
        show_service_message( '申请提交出现了异常，请稍后重试', '', '', 'info' , true );
    }
}
/**
 * 获取全部服务订单
 */
elseif($action == "aftermarket_list" ){
    ...
    $aftermarket = get_user_aftermarket($user_id, $pager['size'], $pager['start']);
    
    $smarty->assign('orders', $aftermarket);
 ...
 * 售后服务详情
 */
elseif( $action == 'aftermarket_detail'){
    ...
    if ($ret_id == 0) {
        ecs_header("Location: user.php?act=aftermarket_list");
    }
    /* 操作记录 */
    $action_list = get_return_action($ret_id);
    $smarty->assign('action_list', $action_list);
    /* 订单详情 */
    $order = get_aftermarket_detail($ret_id, $user_id);
    $order['progress'] = get_aftermarket_progress($order);//订单进程
    $order['handler'] = get_return_operate($order); //可执行操作
    /*服务订单 订单状态 退款状态 审核状态 语言项*/ 
    if ($order['return_status'] == RF_APPLICATION && $order['is_check'] == RC_APPLY_FALSE) {
        /* 状态 ： 待审核 */
        $order['return_status'] = $_LANG['wait_check'];
    } elseif ($order['return_status'] == RF_APPLICATION && $order['is_check'] == RC_APPLY_SUCCESS) {
        /* 状态 ： 审核成功 */
        $order['return_status'] = $_LANG['check_success'];
    } elseif ($order['return_status'] == RF_APPLY_FALSE && $order['is_check'] == RC_APPLY_FALSE) {
        /* 状态 ： 审核失败 */
        $order['return_status'] = $_LANG['check_false'];
    } elseif ($order['return_status'] == RF_CANCELED) {
        /* 状态 ： 撤销申请 */
        $order['return_status'] = $_LANG['cancel'];
    } elseif ($order['refund_status'] == FF_REFUND && $order['is_check'] == RC_APPLY_SUCCESS) {
        /* 状态 ： 已退款 */
        $order['return_status'] = $_LANG['refund_success'];
    }else {
        $order['return_status'] = $_LANG['rf'][$order['return_status']];
    }
    /*服务详情*/
    $service = get_service_info($order['service_id']);
    $smarty->assign('service', $service);
    $smarty->assign('goods_list', get_return_goods($ret_id));
    $smarty->assign('order', $order);
    $smarty->display('user_transaction.dwt');
}
/**
...
elseif($action == 'cancel_service'){ 
    ...
    if (cancel_service($ret_id, $user_id))
    {
        return_action($ret_id, RF_CANCELED,  FF_NOREFUND, RC_APPLY_FALSE,  $_LANG['cancel_service'] , $_LANG['buyer'], '', $_LANG['cancel_service']); 
        ecs_header("Location: user.php?act=aftermarket_list");
        exit;
    }
    else
    {
        $err->show($_LANG['aftermarket_list_lnk'], 'user.php?act=aftermarket_list');
    }
}
/**
 * 去退货
 */
elseif( $action == 'to_return'){
    
    include_once(ROOT_PATH . 'includes/lib_transaction.php');
    include_once(ROOT_PATH . 'includes/lib_order.php');
    $ret_id = isset($_REQUEST['ret_id']) ? intval($_REQUEST['ret_id']) : 0;
    
    $shipping_id = $_REQUEST['back_shipping_name'];
    /* 获取快递名称 */
    $shipping = shipping_info($shipping_id);
    //将用户订单设置为取消
    $sql = "UPDATE " . $ecs->table('order_return') . " SET return_status = '" . RF_SEND_OUT . "',back_shipping_name = '". $shipping['shipping_name']."',back_other_shipping = '". $_POST['back_other_shipping']."' ,back_invoice_no = '".$_POST['back_invoice_no']."' WHERE ret_id = '$ret_id'";

    if ($db->query($sql)) {
        /* 记录log */
        return_action($ret_id, RF_SEND_OUT,  FF_NOREFUND, RC_APPLY_SUCCESS,  $_LANG['send_out'] , $_LANG['buyer'], '', $_LANG['send_out']);
        show_message($_LANG['edit_shipping_success'], '', 'user.php?act=aftermarket_detail&ret_id='.$ret_id);
        exit;
    } else {
        $err->show($_LANG['aftermarket_list_lnk'], 'user.php?act=aftermarket_detail&ret_id='.$ret_id);
    }
}
?>
