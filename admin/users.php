...
//hzh s
	$sql1 = "UPDATE `ydcom_users` ,\n"
		. "`ydcom_wechat_user` SET `ydcom_users`.wx_nickname = `ydcom_wechat_user`.nickname WHERE `ydcom_users`.user_id = `ydcom_wechat_user`.ect_uid;";
	$db->query($sql1);
		
	$sql2 = "UPDATE `ydcom_users` ,\n"
		. "`ydcom_wechat_user` SET `ydcom_users`.wx_headimgurl = `ydcom_wechat_user`.headimgurl WHERE `ydcom_users`.user_id = `ydcom_wechat_user`.ect_uid;";
	$db->query($sql2);
		
	$sql3 = "UPDATE `ydcom_users` ,\n"
		. "`ydcom_wechat_user` SET `ydcom_users`.wx_p_nickname = `ydcom_wechat_user`.nickname WHERE `ydcom_users`.parent_id = `ydcom_wechat_user`.ect_uid;";
	$db->query($sql3);
		
	$sql4 = "UPDATE `ydcom_users` ,\n"
		. "`ydcom_wechat_user` SET `ydcom_users`.wx_p_headimgurl = `ydcom_wechat_user`.headimgurl WHERE `ydcom_users`.parent_id = `ydcom_wechat_user`.ect_uid;";
	$db->query($sql4);
	//hzh e
    assign_query_info();
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT u.user_id, u.sex, u.birthday, u.pay_points, u.rank_points, u.user_rank , u.user_money, u.frozen_money, u.credit_line, u.parent_id, u.fenxiao_rank,u.fenxiao_rank_mark,u2.user_name as parent_username, u.qq, u.msn,
...
/*         if ($row['fenxiao_rank']==1){   
        $user['fenxiao_rank']='银1';
        }elseif ($row['fenxiao_rank']==2){
        $user['fenxiao_rank']='银1';
        }elseif ($row['fenxiao_rank']==1){
        	$user['fenxiao_rank']='银1';
        }elseif ($row['fenxiao_rank']==1){
        	$user['fenxiao_rank']='银1';
        }elseif ($row['fenxiao_rank']==1){
        ...
        $user['fenxiao_rank_name']=array();  // by fc
        $res_rank_name=mysql_query("select rank_name from  " .$ecs->table('user_yongjin'). " order by rank_id asc");
        while ($row_rank_name=mysql_fetch_array($res_rank_name)){
        	$user['fenxiao_rank_name'][]=$row_rank_name[0];
        }
        foreach ($user['fenxiao_rank_name'] as $value){     	
        }   // by fc
        $user['fenxiao_rank_mark']=$row['fenxiao_rank_mark'];
        $user['fenxiao_rank']=$row['fenxiao_rank'];
...
elseif ($_REQUEST['act'] == 'update')
{
	...
	$fenxiao_rank_last=$db->getOne("select fenxiao_rank from ydcom_users where user_id='$user_id'");//by fc
	$fenxiao_rank=$_POST['fenxiao_rank'];
	if ($fenxiao_rank_last!=$fenxiao_rank){
		$db->query("update  ydcom_users  set fenxiao_rank=$fenxiao_rank,fenxiao_rank_mark=1  where user_id='$user_id'");
	}
	
	//跟新标记
	$fenxiao_rank_mark=$_POST['suosi'];
	$db->query("update  ydcom_users  set fenxiao_rank_mark=$fenxiao_rank_mark  where user_id='$user_id'");
	//by  fc
...
function user_list()
{
    ...
    if ($result === false)
    {
		...
		$filter['member_id'] = empty($_REQUEST['member_id']) ? 0 : intval($_REQUEST['member_id']);//hzh
		...
		if ($filter['member_id'])
        {
            $ex_where .=" AND user_id = '$filter[member_id]' ";
        }
		//hzh e
        ...
        $sql = "SELECT user_id, parent_id, msn, user_name, email, is_validated, user_money, frozen_money, rank_points, pay_points, reg_time, wx_nickname, wx_headimgurl, wx_p_nickname, wx_p_headimgurl ".//hzh
    ...
    for ($i=0; $i<$count; $i++)
    {
        $user_list[$i]['reg_time'] = local_date('Y-m-d H:i', $user_list[$i]['reg_time']);
    }
