...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_account'). " ua left join ".
        		$GLOBALS['ecs']->table('users') . " u on ua.user_id = u.user_id " . $where;//hzh
    ...
    if (isset($_POST['export']))
	{
        ...
        $PHPExcel->getActiveSheet()->setTitle("提现列表");
        //填入表头主标题
        $PHPExcel->getActiveSheet()->setCellValue('A1', $_CFG['shop_name'].'会员提现列表');
        //填入表头副标题
        ...
        //表格宽度
        $PHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(10);//id
		$PHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(10);//序号
        $PHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(20);//收款人姓名
        $PHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(30);//银行名称
        $PHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(30);//银行卡号
        $PHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(20);//付款金额
        $PHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(30);//到账时间
        $PHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(50);//备注

        //表格标题
        $PHPExcel->getActiveSheet()->setCellValue('A3', 'ID');
		$PHPExcel->getActiveSheet()->setCellValue('B3', '序号');
        $PHPExcel->getActiveSheet()->setCellValue('C3', '收款人姓名');
        $PHPExcel->getActiveSheet()->setCellValue('D3', '银行名称');
        $PHPExcel->getActiveSheet()->setCellValue('E3', '银行卡号');
        $PHPExcel->getActiveSheet()->setCellValue('F3', '付款金额');
        $PHPExcel->getActiveSheet()->setCellValue('G3', '0(第二天到账)/1(2小时到账)');
        $PHPExcel->getActiveSheet()->setCellValue('H3', '备注');
...
echo $item[ 'bank_name'];
		...
		foreach ($id_list as $id) {
            ...
            $PHPExcel->getActiveSheet()->setCellValue('A' . ($hang), '1');
			$PHPExcel->getActiveSheet()->setCellValue('B' . ($hang), '1');
            $PHPExcel->getActiveSheet()->setCellValue('C' . ($hang), '张三丰');
            $PHPExcel->getActiveSheet()->setCellValue('D' . ($hang), '中国工商银行');
            $PHPExcel->getActiveSheet()->setCellValue('E' . ($hang), '\''.'6222222222222222222');
            $PHPExcel->getActiveSheet()->setCellValue('F' . ($hang), '10');
            $PHPExcel->getActiveSheet()->setCellValue('G' . ($hang), '0');
            $PHPExcel->getActiveSheet()->setCellValue('H' . ($hang), '备注备注备注备注备注备注备注备注');
        ...
        $Writer->save(str_replace('.php', '.csv', __FILE__));
        $url = "user_account.csv";
        ecs_header("Location: $url\n");
        exit;
    }
    /* 导出功能结束 */
