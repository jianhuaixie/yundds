...
elseif ($_REQUEST['act'] == 'signin')
{
    ...
    if ($row)
    {
		...
		// 青蜂网络0769web.net  商家入驻 检查是否为入驻商的管理员 所属入驻商家是否有效
        if ($row['seller_id']>0)
        {
...
elseif ($_REQUEST['act'] == 'insert')
{
    ...
    $sql = "INSERT INTO ".$ecs->table('admin_user')." (user_name, mobile_phone, email, password, add_time, nav_list, action_list, role_id) ".//hzh
           "VALUES ('".trim($_POST['user_name'])."', '".trim($_POST['mobile_phone'])."', '".trim($_POST['email'])."', '$password', '$add_time', '$row[nav_list]', '$action_list', '$role_id')";//hzh
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    /* 获取管理员信息 */
    $sql = "SELECT user_id, user_name, mobile_phone, email, password, agency_id, role_id FROM " .$ecs->table('admin_user').//hzh
...
elseif ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self')
{
	...
	$admin_mobile_phone  = !empty($_REQUEST['mobile_phone']) ? trim($_REQUEST['mobile_phone']) : '';//hzh
    ...
    if($pwd_modified)
    {
        $sql = "UPDATE " .$ecs->table('admin_user'). " SET ".
			   ...
			   "mobile_phone = '$admin_mobile_phone', ".//hzh
               "email = '$admin_email', ".
               "ec_salt = '$ec_salt' ".
               $action_list.
               $role_id.
...
elseif ($_REQUEST['act'] == 'modif')
{
    ...
    /* 获得当前管理员数据信息 */
    $sql = "SELECT user_id, user_name, mobile_phone, email, nav_list ".//hzh
...
{
    $list = array();
    $sql  = 'SELECT au.user_id, au.user_name, au.mobile_phone, au.email, au.add_time, au.last_login, au.role_id, r.role_name './/hzh
            'FROM ' .$GLOBALS['ecs']->table('admin_user').'  AS au LEFT JOIN ' . $GLOBALS['ecs']->table('role') . ' AS r ON au.role_id = r.role_id ORDER BY user_id DESC';//hzh
