...
if ($_REQUEST['act'] == 'list')
{
	...
	foreach($type_id as $k => $v){
		if($v["send_type"] == 4 || $v["send_type"] == 5){
			$user_bonus .= $v['type_id'].',';
		}
	}
	if($user_bonus){
		$user_bonus = trim($user_bonus,",");
	
		$res = $db->getAll("SELECT bonus_type_id,bonus_sn FROM " .$ecs->table('user_bonus'). " WHERE bonus_type_id in ({$user_bonus}) and user_id=0");
	}

    $smarty->assign('url',    $res);
...
if ($_REQUEST['act'] == 'send')
{
    ...
    }else if($_REQUEST['send_by'] == 5){
	
		 $smarty->assign('type_list',    get_bonus_type_f());
...
//-- 产生领取红包链接
/*------------------------------------------------------ */
if ($_REQUEST['act'] == 'gen_link')
{
//user.php?act=act_add_bonus&bonus_sn=1666777486
   ...
   // header("Content-Disposition: attachment; filename=$bonus_filename.xls");
    ...
    if (EC_CHARSET != 'gbk')
    {
       ...
       // echo ecs_iconv('UTF8', 'GB2312', $_LANG['bonus_sn']) ."\t";
          //echo '红包领取链接' ."\t";
          //$eddy_word="红包领取链接";
          //echo ecs_iconv('UTF8', 'GB2312', $eddy_word ."\t";
    ...
    while ($val = $db->fetchRow($res))
    {
    	...
    	// $result_url='<a href="user.php?act=act_add_bonus&bonus_sn=">'.$val['bonus_sn'] . "</a>";
       // echo '<a href="user.php?act=act_add_bonus&bonus_sn=">'.$val['bonus_sn'] . "</a>\t";
       // echo $val['type_money'] . "\t";
        if (!isset($code_table[$val['type_name']]))
        {
            ...
            else
            {
              ...
              // $code_table[$val['type_name']] = 'dgdgdg';
            }
       ...
       $smarty->assign('result_money',   $val['type_money']);
    $smarty->assign('result_url',   $val['bonus_sn']);
 // $smarty->assign('result_url',   array('result_url' => $val['bonus_sn']));
      $smarty->display('eddy_result_url.htm');
