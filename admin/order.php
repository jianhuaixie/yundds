...
if($_GET['order_ajax'] == 1){
	$sql = "UPDATE " . $ecs->table('order_info') . " SET  shipping_name ='".$_POST['shipping_name']."' WHERE order_sn=".$_POST['order_sn'];
	$res = $db ->query($sql);
	exit;
}
/*------------------------------------------------------ */
...
elseif ($_REQUEST['act'] == 'list')
{
	...
	$smarty->assign('cs_shipped',    CS_SHIPPED);//hzh
...
//青蜂网络0769web.net 商家入驻start 订单查询导出cvs格式
...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    else
    {
        ...
        /* 显示模板 */
		$smarty->assign('postcom', $_LANG['postcom']);
        assign_query_info();
...
elseif ($_REQUEST['act'] == 'delivery_ship')
{
    ...
    if ($order_finish)
    {
        ...
        /* 如果需要，发短信 */
        if ($GLOBALS['_CFG']['sms_order_shipped'] == '1' && $order['tel'] != '')
        {
            include_once('../includes/cls_sms.php');
            $sms = new sms();
            $sms->send($order['tel'], sprintf($GLOBALS['_LANG']['order_shipped_sms'], $order['order_sn'], local_date($GLOBALS['_LANG']['sms_time_format'])));
        }
    }
...
/*------------------------------------------------------ */
    elseif (isset($_POST['to_shipping']))
	{
       ...
       if (!empty($invoice_no))
        ...
        $res_goods_amount=mysql_query("select goods_amount,user_id from ydcom_order_info where order_id=$order_id");
        while ($row_goods_amount=mysql_fetch_array($res_goods_amount)){
        	$goods_amount_fc=$row_goods_amount[0];
        	$user_id_f=$row_goods_amount[1];
        }
        //$user_id= user_info($order['user_id']);
        //$user_id1=$user_id;
        $res_rank_points=mysql_query("select rank_points from ydcom_users where user_id=$user_id_f");
        while ($row_rank_points=mysql_fetch_array($res_rank_points)){
        	$rank_points_fc1=$row_rank_points[0];
        }
        $rank_points_fc=$rank_points_fc1-$goods_amount_fc;
        //echo "商品价".$goods_amount_fc."原等级积分".$rank_points_fc1."后等级积分".$rank_points_fc."uid".$user_id."uidj".$user_id1;
        mysql_query("update ydcom_users set rank_points=$rank_points_fc where user_id=$user_id_f")or die("shibai");
...
/*------------------------------------------------------ */
    ...
    elseif (isset($_POST['export']))
	{
        ...
        //设置excel属性基本信息
        $PHPExcel->getProperties()->setCreator("云的商城")
        ->setLastModifiedBy("云的商城")
        ->setTitle("云的商城")
        ->setSubject("订单列表")
        ->setDescription("")
        ->setKeywords("订单列表")
        ...
        $PHPExcel->getActiveSheet()->getColumnDimension('F')->setWidth(10);//配送方式
$PHPExcel->getActiveSheet()->getColumnDimension('G')->setWidth(10);//配送费用
        $PHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(10);//收件人
        $PHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(35);//收货地址
        $PHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(15);//电话
        $PHPExcel->getActiveSheet()->getColumnDimension('K')->setWidth(15);//手机
        $PHPExcel->getActiveSheet()->getColumnDimension('L')->setWidth(15);//货号
        $PHPExcel->getActiveSheet()->getColumnDimension('M')->setWidth(15);//属性
$PHPExcel->getActiveSheet()->getColumnDimension('N')->setWidth(15);//单价
        $PHPExcel->getActiveSheet()->getColumnDimension('O')->setWidth(6);//数量
$PHPExcel->getActiveSheet()->getColumnDimension('P')->setWidth(15);//小计
		$PHPExcel->getActiveSheet()->getColumnDimension('Q')->setWidth(15);//品牌
		$PHPExcel->getActiveSheet()->getColumnDimension('R')->setWidth(35);//商品名称
		$PHPExcel->getActiveSheet()->getColumnDimension('S')->setWidth(15);//本单总金额
        ...
        $PHPExcel->getActiveSheet()->setCellValue('F3', '配送方式');
$PHPExcel->getActiveSheet()->setCellValue('G3', '配送费用');
        $PHPExcel->getActiveSheet()->setCellValue('H3', '收件人');
        $PHPExcel->getActiveSheet()->setCellValue('I3', '收货地址');
        $PHPExcel->getActiveSheet()->setCellValue('J3', '电话');
        $PHPExcel->getActiveSheet()->setCellValue('K3', '手机');
        $PHPExcel->getActiveSheet()->setCellValue('L3', '货号');
        $PHPExcel->getActiveSheet()->setCellValue('M3', '属性');
$PHPExcel->getActiveSheet()->setCellValue('N3', '单价');
        $PHPExcel->getActiveSheet()->setCellValue('O3', '数量');
		$PHPExcel->getActiveSheet()->setCellValue('P3', '小计');
		$PHPExcel->getActiveSheet()->setCellValue('Q3', '品牌');
		$PHPExcel->getActiveSheet()->setCellValue('R3', '商品名称');
	$PHPExcel->getActiveSheet()->setCellValue('S3', '总金额');
        ...
        foreach ($order_sn_list as $order_sn) {
            ...
            while ($row = $db->fetchRow($res)) {
                ...
                //$row['formated_subtotal']    = price_format($row['goods_price'] * $row['goods_number']);
				//$row['formated_subtotal'] = strip_tags(price_format($row['goods_price'] * $row['goods_number']));//hzh
				$row['formated_subtotal'] = $row['goods_price'] * $row['goods_number'];//hzh
				//$row['goods_price'] = strip_tags(price_format($row['goods_price']));//hzh
                $row['formated_goods_price'] = price_format($row['goods_price']);
				
                //var_dump($order);die;
                //输出订单的商品，由于可能一个人购买多个商品，所以在这先输出了
                $PHPExcel->getActiveSheet()->setCellValue('L' . $chanpin, $row['goods_sn']);
                $PHPExcel->getActiveSheet()->setCellValue('M' . $chanpin, $row['goods_attr']);
				$PHPExcel->getActiveSheet()->setCellValue('N' . $chanpin, $row['goods_price']);
                $PHPExcel->getActiveSheet()->setCellValue('O' . $chanpin, $row['goods_number']);
				$PHPExcel->getActiveSheet()->setCellValue('P' . $chanpin, $row['formated_subtotal']);//小计
				$PHPExcel->getActiveSheet()->setCellValue('Q' . $chanpin, $row['brand_name']);
				$PHPExcel->getActiveSheet()->setCellValue('R' . $chanpin, $row['goods_name']);
				$PHPExcel->getActiveSheet()->setCellValue('S' . ($hang), $order['goods_amount']+$order['shipping_fee']);//本单总计：商品总价+运费
            ...
            $PHPExcel->getActiveSheet()->setCellValue('E' . ($hang), $order['pay_name']);
            $PHPExcel->getActiveSheet()->setCellValue('F' . ($hang), $order['shipping_name']);
			$PHPExcel->getActiveSheet()->setCellValue('G' . ($hang), $order['shipping_fee']);
            $PHPExcel->getActiveSheet()->setCellValue('H' . ($hang), $order['consignee']);
            $PHPExcel->getActiveSheet()->setCellValue('I' . ($hang), str_replace(" ","",$order['region']).$order['address']);
            $PHPExcel->getActiveSheet()->setCellValue('J' . ($hang), $order['tel']);
            $PHPExcel->getActiveSheet()->setCellValue('K' . ($hang), $order['mobile']);
...
function order_list()
{
    ...
    if ($result === false)
    {
        ...
        switch($filter['composite_status'])
        {
			...
			//hzh
			case CS_SHIPPED :
                $where .= order_query_sql('has_shipped');
                break;
